...
get.rcm.delta <- function(var.name,rcm.file,past.int,proj.int,std=FALSE) {
   ...
   past.st <- head(grep(past.bnds[1],time),1)
   past.en <- tail(grep(past.bnds[2],time),1)
   ...
   proj.st <- head(grep(proj.bnds[1],time),1)
   proj.en <- tail(grep(proj.bnds[2],time),1)
...
morph.tas <- function(gcm,past.int,proj.int,
   ...
   tmy.file <- paste0(tmy.dir,"tas_CWEC_TMY_CanESM2+CRCM5_historical_",past.int,".nc")
   new.tmy.file <- paste0(tmy.dir,"morphed_tas_CWEC_TMY_ERA-Interim+",gcm,"+CRCM5_historical+rcp85_",proj.int,".nc")
...
##      tmp.delta <- array(tas.delta[,,mn],c(41,41,sum(ix)))
##      tmp.alpha <- array(tas.alpha[,,mn],c(41,41,sum(ix)))
##      tmp.month <- array(tmy.monthly.tas[,,mn],c(41,41,sum(ix)))
...
##      tmp.delta <- array(tas.delta[,,wk],c(41,41,sum(ix)))
##      tmp.alpha <- array(tas.alpha[,,wk],c(41,41,sum(ix)))
##      tmp.week <- array(tmy.weekly.tas[,,wk],c(41,41,sum(ix)))
...
##   }
   ...
   for (dy in 1:365) {
      ...
      tmp.delta <- array(tas.delta[,,dy],c(41,41,sum(ix)))
      tmp.alpha <- array(tas.alpha[,,dy],c(41,41,sum(ix)))
      tmp.day <- array(tmy.daily.tas[,,dy],c(41,41,sum(ix)))
...
##      ix <- tmy.daily.fac == paste0(sprintf('%03d',dy))
##      tmp.delta <- array(tas.delta[,,dy],c(41,41,sum(ix)))
##      tmp.alpha <- array(tas.sd[,,dy],c(41,41,sum(ix)))
##      tmp.day <- array(tmy.daily.tas[,,dy],c(41,41,sum(ix)))
...
##morph.tmy.file <- function(var.name,gcm) {
   ...
   base.dir <- '/storage/data/climate/downscale/RCM/CRCM5/reconfig/'
