...
correct.rcm.time.series <- function(nc,freq) {
  ...
  fixed.series <- new.origin + time.values*mulc
...
move.data.to.new.file <- function(gcm,varname,
  ...
  ncvar_put(nc,varid='lon',vals=lon[lon.ix,lat.ix])
  ncvar_put(nc,varid='lat',vals=lat[lon.ix,lat.ix])
...
browser()
    ...
    for (i in seq_along(l.st)) {
      ...
      print(l.st[i])
      var.subset <- ncvar_get(hist.nc,varname,start=c(100,50,1,l.st[i]),count=c(100,100,1,l.cnt[i])) 
